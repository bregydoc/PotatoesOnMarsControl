...
public class Sun {
    ...
    public void setDay() {
        try {
            String strResp = "";
            do {
                this.satellite.systemsCommunication.write("D".getBytes(), 1);
                byte[] readBuffer = new byte[2];
                this.satellite.systemsCommunication.read(readBuffer, 2);
    ...
    public void setNight() {
        try {
            String strResp = "";
            do {
                this.satellite.systemsCommunication.write("N".getBytes(), 1);
                byte[] readBuffer = new byte[2];
                this.satellite.systemsCommunication.read(readBuffer, 2);
                strResp = new String(readBuffer, StandardCharsets.UTF_8);
                Thread.sleep(100);
